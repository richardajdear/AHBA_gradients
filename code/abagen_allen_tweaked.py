...
from abagen import (correct, datasets, images, io, matching, probes_, reporting,
               samples_, utils)
from abagen.transforms import xyz_to_ijk
...
def get_expression_data(atlas,
    ...
    for donor, data in files.items():
        ...
        if not_cortex:
            # annot = annot.copy().loc[lambda x: (x['structure_name'].str.contains('cingulate')) | (x['structure'] != 'cortex')]
            annot = annot.copy().loc[lambda x: x['structure'] != 'cortex']
        if only_left:
    ...
    # dictionary to store labels matched for each donor
    labels = {}
    ...
    for subj in microarray:
        ...
        labels[subj] = atlas[subj].label_samples(annotation[subj], tolerance)
        ...
        nz = np.asarray(labels[subj] != 0).squeeze()
        if nz.sum() == 0:
            LGR.warning(f'No samples matched to atlas for donor {subj}')
            microarray[subj].index = labels[subj]['label']
        ...
        if norm_matched:
            ...
            labels[subj] = labels[subj].loc[nz]

        # get counts of samples collapsed into each ROI
        labs, num = np.unique(labels[subj], return_counts=True)
        counts.loc[labs, subj] = num
        LGR.info(f'{counts.iloc[1:][subj].sum():>3} / {len(nz)} '
                 f'samples matched to regions for donor #{subj}')
        
    # find regions where the total samples (from all donors) is above a threshold
    labs_over_samples_threshold = counts[counts.sum(axis=1) >= samples_threshold].index.values
    LGR.info(f'{len(labs_over_samples_threshold)} / {len(all_labels)} '
             f'regions have >={samples_threshold} total samples')
    # find regions where the number of donors represented is above a threshold
    labs_over_donors_threshold = counts[(counts!=0).sum(axis=1) >= donors_threshold].index.values
    LGR.info(f'{len(labs_over_donors_threshold)} / {len(all_labels)} '
             f'regions have samples from >={donors_threshold} donors')
    
    for subj in microarray:
        # filter labels with < x samples matched
        if samples_threshold > 0:
            mask = np.isin(labels[subj], labs_over_samples_threshold)
            microarray[subj] = microarray[subj].loc[mask]
            annotation[subj] = annotation[subj].loc[mask]
            labels[subj] = labels[subj].loc[mask]
        # filter labels with < x donors matched
        if donors_threshold > 0:
            mask = np.isin(labels[subj], labs_over_donors_threshold)
            microarray[subj] = microarray[subj].loc[mask]
            annotation[subj] = annotation[subj].loc[mask]
            labels[subj] = labels[subj].loc[mask]
        ...
        if missing is not None:
            if missing == 'interpolate':
                exp, lab = _interpolate_missing(atlas[subj], labels[subj],
                                                microarray[subj],
                                                annotation[subj])
                microarray[subj] = microarray[subj].append(exp)
                labels[subj] = labels[subj].append(lab)
            elif missing == 'centroids':
                centroids += [_match_centroids(atlas[subj], labels[subj],
                                               microarray[subj],
                                               annotation[subj])]

        microarray[subj] = pd.merge(microarray[subj], labels[subj], on='sample_id') \
                             .set_index('label') \
                             .rename_axis('gene_symbol', axis=1)
        
        # set index of labels to well_id for output
        labels[subj].index = annotation[subj]['well_id']
    ...
    # aggregate labels across donors
    labels = pd.concat(labels).reset_index(level=0,drop=True)['label']
