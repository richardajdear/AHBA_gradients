...
def get_var_explained(version):
    """
    Compute variance explained of components by sucessively computing total variance of residuals
    """
...
def get_coexp(expression, order='', return_order = False):
    """
    Compute region-by-region co-expression matrix
    """
...
def get_graph_metric(coexp, metric='transitivity', threshold=None, nonnegative=True):
    """
    Compute clustering coefficient of co-expression matrix
    """
    # sp = sparse.csr_matrix(coexp+1)
...
def get_moran_I(scores,
    ...
    """
    Compute Moran's autocorrelation score
    """
...
###########################################################
### LEGACY - following functions not used in final analysis
###########################################################
